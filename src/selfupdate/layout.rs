...
pub const REPO: &str = "CodeVachon/family-chat-cli";
/// Name of the executable (without `.exe`).
pub const BIN_NAME: &str = "family-chat-cli";
/// Directory under `$HOME` that holds the managed installation.
pub const ROOT_DIR_NAME: &str = ".family-chat-cli";
...
pub struct HostTarget {
    ...
    pub arch: Arch,
    /// Release asset name without the `.gz`, e.g. `family-chat-cli-darwin-arm64`.
    pub binary_name: String,
    /// Release asset name as published, e.g. `family-chat-cli-darwin-arm64.gz`.
...
/// The five combinations the release workflow builds. Linux is glibc, not musl: `keyring`'s
/// Secret Service backend links the system `libdbus` via FFI (`libdbus-sys`), which a musl
/// static build can't satisfy the way a dependency-free tool could.
...
/// The installation root: `$FAMILY_CHAT_CLI_INSTALL_DIR`, else `~/.family-chat-cli`.
pub fn install_root() -> Option<PathBuf> {
    if let Some(dir) = env::var_os("FAMILY_CHAT_CLI_INSTALL_DIR").filter(|v| !v.is_empty()) {
        return Some(PathBuf::from(dir));
    }
    dirs::home_dir().map(|home| home.join(ROOT_DIR_NAME))
}

/// Directories that may hold the `family-chat-cli` symlink on PATH.
pub fn candidate_bin_dirs() -> Vec<PathBuf> {
    let mut dirs_out = Vec::new();
    if let Some(dir) = env::var_os("FAMILY_CHAT_CLI_BIN_DIR").filter(|v| !v.is_empty()) {
...
///
/// Symlinks are resolved, so invoking through `~/.local/bin/family-chat-cli` still reports the
...
///
/// `FAMILY_CHAT_CLI_DOWNLOAD_BASE` overrides the GitHub URL so CI can verify an installer against
/// locally served assets before they are published.
pub fn download_base(version: &str) -> String {
    if let Ok(base) = env::var("FAMILY_CHAT_CLI_DOWNLOAD_BASE")
        && !base.is_empty()
    {
...
mod tests {
    ...
    fn host_targets_name_assets_like_the_release_workflow() {
        ...
        assert_eq!(t.asset_name, "family-chat-cli-darwin-arm64.gz");
        assert_eq!(t.exe_name(), "family-chat-cli");
        let t = host_target_for("linux", "x86_64").unwrap();
        assert_eq!(t.asset_name, "family-chat-cli-linux-x64.gz");
        let t = host_target_for("windows", "x86_64").unwrap();
        assert_eq!(t.binary_name, "family-chat-cli-windows-x64.exe");
        assert_eq!(t.asset_name, "family-chat-cli-windows-x64.exe.gz");
        assert_eq!(t.exe_name(), "family-chat-cli.exe");
    }
    ...
    fn download_base_points_at_the_github_release_by_default() {
        ...
        if env::var_os("FAMILY_CHAT_CLI_DOWNLOAD_BASE").is_none() {
            assert_eq!(
                download_base("0.1.0"),
                "https://github.com/CodeVachon/family-chat-cli/releases/download/v0.1.0"
            );
        }
    }
    ...
    fn install_sh_agrees_with_this_layout() {
        ...
        assert!(
            script.contains("FAMILY_CHAT_CLI_DOWNLOAD_BASE"),
            "download base override"
        );
        assert!(
            script.contains("FAMILY_CHAT_CLI_INSTALL_DIR"),
            "install dir override"
        );
        assert!(
            script.contains("FAMILY_CHAT_CLI_BIN_DIR"),
            "bin dir override"
        );
        assert!(
            script.contains("FAMILY_CHAT_CLI_VERSION"),
            "version override"
        );

        // The script builds `family-chat-cli-<os>-<arch>`; each os/arch token must be present.
