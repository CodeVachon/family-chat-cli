...
/// Default GitHub API host; `FAMILY_CHAT_CLI_API_BASE` overrides it for tests and mirrors.
...
pub enum ReleaseError {
    ...
    #[error(
        "GitHub API rate limit reached. Unauthenticated requests are capped at 60 per hour per IP. \
         Retry later, or pass a version explicitly: family-chat-cli upgrade 0.1.0"
    )]
    RateLimited,
    #[error(
        "the GitHub API returned {0} when listing releases. Pass a version explicitly to skip the \
         lookup: family-chat-cli upgrade 0.1.0"
    )]
    Status(u16),
...
/// `$FAMILY_CHAT_CLI_API_BASE` or the real GitHub API.
pub fn api_base() -> String {
    env::var("FAMILY_CHAT_CLI_API_BASE")
        .ok()
        .filter(|v| !v.is_empty())
