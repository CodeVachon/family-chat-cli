...
//! Opt out with `FAMILY_CHAT_CLI_NO_UPDATE_CHECK=1`.
...
pub fn disabled() -> bool {
    env::var_os("FAMILY_CHAT_CLI_NO_UPDATE_CHECK").is_some_and(|v| !v.is_empty())
        || env::var_os("CI").is_some_and(|v| !v.is_empty())
}

/// The line to print after the TUI exits, if a newer release is known.
///
...
pub fn maybe_line() -> Option<String> {
    ...
    (compare_versions_descending(&current, &latest) == Ordering::Greater).then(|| {
        format!(
            "family-chat-cli {latest} is available (you have {current}) — family-chat-cli upgrade"
        )
    })
}
