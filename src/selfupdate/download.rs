...
pub fn install_binary(
    ...
    let staging = bin_dir.join(".family-chat-cli.partial");
...
fn point_current_at(root: &Path, version_dir: &Path) -> std::io::Result<()> {
    ...
    #[cfg(windows)]
    {
        ...
        // current\bin\family-chat-cli.exe exists either way.
...
mod tests {
    ...
    fn checksum_lookup_tolerates_one_or_two_spaces_and_case() {
        let checksums =
            "ABCDEF  family-chat-cli-linux-x64.gz\n012345 family-chat-cli-darwin-arm64.gz\n";
        assert_eq!(
            expected_checksum(checksums, "family-chat-cli-linux-x64.gz").as_deref(),
            Some("abcdef")
        );
        assert_eq!(
            expected_checksum(checksums, "family-chat-cli-darwin-arm64.gz").as_deref(),
            Some("012345")
        );
        assert_eq!(
            expected_checksum(checksums, "family-chat-cli-windows-x64.exe.gz"),
            None
        );
    }
    ...
    fn install_binary_writes_the_version_dir_and_repoints_current() {
        ...
        assert_eq!(exe, root.join("versions/v0.1.0/bin/family-chat-cli"));
        ...
        {
            use std::os::unix::fs::PermissionsExt;
            let mode = fs::metadata(root.join("versions/v0.2.0/bin/family-chat-cli"))
                .unwrap()
                .permissions()
                .mode();
